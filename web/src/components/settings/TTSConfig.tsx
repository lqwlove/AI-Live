  ...
  return (
    <div className="flex flex-col gap-4 rounded-[10px] border border-[var(--border-app)] bg-[var(--bg-card)] px-5 pt-4 pb-5">
      <div className="flex items-center gap-2">
        ...
        <span className="text-sm font-semibold text-[var(--font-primary)]">TTS Configuration</span>
      </div>

      <div className="flex flex-col gap-1.5">
        <label className="text-xs font-medium text-[var(--font-secondary)]">Engine</label>
        <select
        ...
        >
          ...
          <option value="volcengine">Volcengine (声音复刻)</option>
        </select>
      </div>
      ...
      {isVolc ? (
        <>
          ...
          <Field label="App ID" value={volc.app_id as string} onChange={(v) => setVolc("app_id", v)} />
          <Field label="Access Token" value={volc.access_token as string} onChange={(v) => setVolc("access_token", v)} type="password" />
          <Field label="Resource ID" value={volc.resource_id as string} onChange={(v) => setVolc("resource_id", v)} />
        </>
      ) : (
        <Field label="Voice" value={tts.voice as string} onChange={(v) => setTts("voice", v)} />
      )}

      <div className="flex gap-6">
        <div className="flex min-w-0 flex-1 flex-col gap-1.5">
          <div className="flex items-center">
            <span className="text-xs font-medium text-[var(--font-secondary)]">Rate</span>
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-1.5">
          <div className="flex items-center">
            <span className="text-xs font-medium text-[var(--font-secondary)]">Volume</span>
            <span className="min-w-0 flex-1" />
            <span className="text-xs font-medium text-[var(--font-primary)]">
